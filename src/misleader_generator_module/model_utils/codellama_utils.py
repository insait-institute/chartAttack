...
def get_annotations(imgname: str, chart_type: str, partition):
    annotations_path = f'/ukp-storage-1/ortiz/datasets/PlotQA/clean_{partition}/enhanced_simplified_annotations_v2/{chart_type}/{imgname}.json'
    annotations = read_json(annotations_path)
    return annotations

def codellama_inference(args, questions):
    ...
    for item in questions:
        ...
        annotations = get_annotations(item["imgname"], args.chart_type, args.partition)
        ...
        outputs = model.generate(inputs, attention_mask=attention_mask, max_new_tokens=args.max_new_tokens, do_sample=False, pad_token_id=tokenizer.eos_token_id)
